...
**Pitch (2 sentences):**
Word Hunt VS is a live word race where a room full of people on their phones plays one 4x4 board for 75 seconds against an 11-million-parameter model we trained today on a Lambda A100 and Gemma 4 12B served from the same GPU, every AI moving tile by tile through a human-pace hand so nobody can dump forty words in three seconds. At the buzzer Gemma itself calls a Nango tool to post the recap into Discord, every Gemma call is traced in Respan, and the same room with no humans in it is an open-model eval harness (nano 16,550 a board at 99% valid words vs Gemma 12B 1,780 at 20%).
...
Open models ship faster than anyone can evaluate them on tasks that look like the real world: perceive, decide, act under a clock, against people. The game is the acquisition channel and the ground truth; the business is the paired measurement. Every round produces a labelled comparison (valid rate, latency, tokens, cost, word length) of a frontier open model, a distilled on-device policy, and humans on the same board. Model: free-to-play arenas as top of funnel; paid eval seats for model teams (bring your endpoint, get a human-calibrated score with Respan traces and cost per action); a distillation service that trains the nano-sized policy from a customer's teacher model. Every fixed-rule casual game is a new arena and a new benchmark, and game studios pay for believable bots that play at human pace, which is exactly the hand controller.
...
| 2:45–3:00 | Projector: tap Rematch, countdown running. | "11M params, zero tokens, human pace; 12B finds longer words at a thousand times the compute. Nango, Lambda, Respan, Gemma: all live in that round. Questions." |
