...
def gemma_seats_from_env() -> list[dict]:
    ...
    for m in [x.strip() for x in os.environ.get("GEMMA_MODELS", "").split(",") if x.strip()]:
        ...
        size = re.sub(r"^gemma-\d+-(\w+?)-it$", lambda mm: mm.group(1).upper(), short) if short.lower().startswith("gemma") else short
        host = urllib.parse.urlparse(base).hostname or ""
        where = "Lambda" if host.startswith("129.146.") or "lambda" in host else ("laptop" if host in ("localhost", "127.0.0.1") else host)
        out.append({"id": sid if sid.startswith("gemma") else "gemma-" + sid, "name": os.environ.get("GEMMA_SEAT_NAME") or f"Gemma {size} ({where})", "model": m,
