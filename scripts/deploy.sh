...
for v in GEMMA_BASE_URL GEMMA_API_KEY GEMMA_MODELS GEMMA_SEATS NANO_CKPT WH_NANO_TEMPERATURE WH_NANO_THINK WH_SEATS WH_MAX_HUMANS WH_INTEGRATIONS WH_CROWD_THINK WH_MAX_AI_CROWD WH_HOTJOIN_S \
         ...
         RESPAN_ENABLED RESPAN_BASE_URL RESPAN_MODE RESPAN_MODEL RESPAN_CREDENTIAL_OVERRIDE; do
...
if [[ "${WH_INTEGRATIONS:-1}" == "1" ]]; then
  if gcloud secrets versions access latest --secret "$SECRET_NAME" >/dev/null 2>&1; then
    ...
    # Discord links must point at the revision that owns the room: use the tag URL
