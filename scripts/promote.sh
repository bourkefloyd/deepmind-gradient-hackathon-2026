...
# players in a live room on the old revision lose it. Refuse while a room has humans unless FORCE=1.
LIVE="$(curl -fsS "$URL/api/health" 2>/dev/null | python3 -c 'import json,sys;d=json.load(sys.stdin);print(",".join(r["code"] for r in d.get("room_list",[]) if r.get("humans",0)>0 and r.get("state")!="results"))' 2>/dev/null || true)"
